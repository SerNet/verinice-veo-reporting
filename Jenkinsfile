...
pipeline {
    ...
    stages {
        ...
        stage('Dockerimage') {
            ...
            steps {
                script {
                    def dockerImage = docker.build("eu.gcr.io/veo-projekt/veo-reporting:git-${env.GIT_COMMIT}", "--label org.opencontainers.image.version='$projectVersion' --label org.opencontainers.image.revision='$env.GIT_COMMIT' .")
